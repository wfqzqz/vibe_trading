...
## 5. F-04 closure — upgrade re-run + conflict list + lock hash (DORA-147)
...
### 5.1 `numpy>=2` conflict list — **closed (empty)**
...
### 5.2 Reconciliation regression (upgrade re-run target)
...
### 5.3 Lock-file hash verification
...
- Any upgrade to `py-alpha-lib` / `numpy` **must** re-generate the lock
  (`uv pip compile --generate-hashes …`) and re-run §5.2's reconciliation.
